...
namespace Ivayami.Save
{
    ...
    public class DeliverUISave : SaveObject
    {
        ...
        public class Data
        {
            ...
            public string[] RequestsDoneItemIDs;

            public Data(Data data = null)
            {
                ItemsDeliveredIDs = data?.ItemsDeliveredIDs;
                RequestsDoneItemIDs = data?.RequestsDoneItemIDs;
            }
        }
        ...
        protected override void OnEnable()
        {
            ...
            _currentData.RequestsDoneItemIDs = _deliverUI.CurrentRequests.Select(x => x.name).ToArray();
        }
        ...
        public override void SaveData()
        {
            ...
            _newData.RequestsDoneItemIDs = _deliverUI.CurrentRequests.Select(x => x.name).ToArray();
        ...
        {
            if (first.ItemsDeliveredIDs.Length != second.ItemsDeliveredIDs.Length ||
                first.RequestsDoneItemIDs.Length != second.RequestsDoneItemIDs.Length) return false;
            for (int i = 0; i < first.ItemsDeliveredIDs.Length; i++)
            {
                if (first.ItemsDeliveredIDs[i] != second.ItemsDeliveredIDs[i]) return false;
            }
            for (int i = 0; i < first.RequestsDoneItemIDs.Length; i++)
            {
                if (first.RequestsDoneItemIDs[i] != second.RequestsDoneItemIDs[i]) return false;
            }
            return true;
        }
    }
